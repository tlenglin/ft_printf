...
/*   Updated: 2017/01/13 16:30:24 by tlenglin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
unsigned char	*ft_s_conversion(t_print *print, va_list ap, char c)
{
	...
	if (c == 's')
	{
		if (print->l == 1)
		{
			...
			while (nb[i] != 0)
			{
				tmp = (unsigned char*)ft_strjoin((char*)tmp, (char*)ft_get_wildchar(nb[i]));
				i++;
			}
		}
...
unsigned char	*ft_p_conversion(t_print *print, va_list ap)
{
	...
	if (print->zero == 1 || print->hashtag == 1 || print->plus == 1 || print->space == 1)
		return (NULL);
	p = va_arg(ap, void*);
	tmp = (unsigned char*)ft_strjoin("0x", (char*)ft_itoa_ubase((unsigned int)p, 16));
